...
// === Map === intentionally not present.
// Maps render inline via ```map fences (see render-message.ts:renderMapBlocks).
// add_artifact { type: 'map' } returns a structured guidance error pointing
// the agent at the inline path. Legacy map artifacts in old snapshots fall
// through to renderGenericArtifact.
